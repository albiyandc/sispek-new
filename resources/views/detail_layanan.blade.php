...
<div class="bg-[#fcfdfd] min-h-screen pb-20 pt-6">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 fade-in-up">
        ...
        <div class="text-[10px] sm:text-xs text-gray-500 mb-6 flex items-center gap-1.5 font-medium overflow-x-auto whitespace-nowrap pb-1">
            ...
            <span>&rsaquo;</span> 
            <a href="#" class="hover:text-blue-600 transition">{{ $layanan->nama_kecamatan ?? 'Cihideung' }}</a> 
            <span>&rsaquo;</span> 
        ...
        <div class="flex items-start gap-4 mb-8">
            <div class="bg-[#dbe5fa] text-[#0b53c8] w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center shrink-0 shadow-sm">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 border-t-4 border-t-[#0b53c8] p-6 sm:p-8 mb-8">
            <div class="flex items-center gap-2.5 mb-5">
                <svg class="w-5 h-5 text-[#0b53c8]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 022 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"></path>
                </svg>
            ...
            <div class="space-y-3.5 text-xs sm:text-sm text-gray-700">
                ...
                @foreach($persyaratanList as $item)
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <span class="leading-relaxed text-gray-700">{{ trim($item) }}</span>
                    </div>
                    @endif
                @endforeach
            </div>
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <!-- Waktu Pelayanan -->
                <div class="bg-[#f0f4fc] rounded-2xl p-5 flex flex-col justify-start">
                    <div class="text-blue-600 mb-3">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    </div>
                    <div class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">WAKTU PELAYANAN</div>
                    <div class="font-bold text-gray-900 text-xs sm:text-sm">{{ $layanan->waktu_penyelesaian ?? '3–5 Hari Kerja' }}</div>
                </div>
                ...
                <div class="bg-[#f0f4fc] rounded-2xl p-5 flex flex-col justify-start">
                    ...
                    <div class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">BIAYA / TARIF</div>
                ...
                <div class="bg-[#f0f4fc] rounded-2xl p-5 flex flex-col justify-start">
                    ...
                    <div class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">PRODUK AKHIR</div>
                    <div class="font-bold text-gray-900 text-xs sm:text-sm">{{ $layanan->produk_pelayanan ?? 'Dokumen KK (PDF/Fisik)' }}</div>
                </div>
                ...
                <div class="bg-[#f0f4fc] rounded-2xl p-5 flex flex-col justify-start">
                    ...
                    <div class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">JUMLAH PELAKSANA</div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 border-t-4 border-t-emerald-600 p-6 sm:p-8 mb-8">
            ...
            <div class="relative border-l-2 border-emerald-600 ml-3 space-y-6 sm:space-y-8 pl-6">
                
                <!-- Langkah 1 -->
                <div class="relative">
                    <div class="absolute -left-[31px] bg-emerald-600 w-3.5 h-3.5 rounded-full border-2 border-white shadow-sm mt-0.5"></div>
        ...
        <div class="bg-[#fef2f2] rounded-2xl border border-red-100 p-6 sm:p-8">
            ...
            <div class="space-y-3 text-xs sm:text-sm text-gray-700 font-medium">
                <div class="flex items-center gap-3">
                    <svg class="w-4 h-4 text-[#c5221f] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                    <span>(0265) 123456 - Ext. 102</span>
                </div>
                <div class="flex items-center gap-3">
                    <svg class="w-4 h-4 text-[#c5221f] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                    <span>pengaduan.cihideung@tasikmalayakota.go.id</span>
                </div>
                <div class="flex items-center gap-3">
                    <svg class="w-4 h-4 text-[#c5221f] shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.198-.198.347-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/></svg>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 mt-8">
            <div class="flex items-center gap-2 mb-4">
                <span class="material-symbols-outlined text-[#0b53c8]">domain</span>
                <h2 class="font-bold text-gray-900 text-base sm:text-lg">Layanan Serupa Tersedia Di</h2>
            </div>
            <p class="text-xs sm:text-sm text-gray-500 mb-4">
                Layanan <span class="font-bold text-gray-800">{{ $layanan->nama_layanan ?? '' }}</span> ini juga diselenggarakan secara resmi di kecamatan/instansi berikut:
            </p>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                @foreach($layananSerupa as $serupa)
                <a href="{{ route('layanan.detail', $serupa->id) }}" class="flex items-center gap-3 p-3 rounded-xl bg-blue-50/50 hover:bg-blue-50 border border-blue-100/50 transition group">
                    <div class="w-8 h-8 rounded-full bg-[#0b53c8] text-white flex items-center justify-center font-bold text-xs shrink-0">
                        {{ substr($serupa->instansi->nama_instansi ?? 'I', 0, 1) }}
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="font-bold text-xs text-gray-900 group-hover:text-[#0b53c8] transition-colors truncate">
                            {{ $serupa->instansi->nama_instansi ?? 'Instansi' }}
                        </div>
                        <div class="text-[10px] text-gray-500">Lihat Persyaratan &rsaquo;</div>
                    </div>
                </a>
                @endforeach
            </div>
